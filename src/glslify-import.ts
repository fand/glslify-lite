...
function glslifyImport(file: string, src: string, opts: any, done: any) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        (function(i) {
            ...
            var imported = /#pragma glslify:\s*import\(([^\)]+)\)/.exec(
                token.data
            );
            if (!imported) return;
            ...
            if (typeof done === "function") {
                resolve(target, { basedir: basedir }, function(err, resolved) {
                    ...
                    fs.readFile(resolved, "utf8", function(err, contents) {
                        ...
                        contents = modifyRequirePaths(
                            contents,
                            basedir,
                            target
                        );

                        glslifyImport(resolved, contents, opts, function(
                            err,
                            contents
                        ) {
                            if (err) return done(err);
